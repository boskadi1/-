...
v1.52 在 v1.51 的卸货流程基础上加入车体左右搜索：

- 摄像头进入 `10°/30°` 后保持固定，云台左右舵机不扫描。
- 云台到位后先以左轮70、右轮60慢速直行1秒。
- 1秒内没有识别到圆块，车体先原地左转350 ms，再右转700 ms。
- 此后车体以700 ms为一段左右交替原地扫转。
- 原地转向复用原有转向函数和左92、右70的转向参数。
- 任意搜索阶段识别到圆块都会立即停止扫转并进入视觉对齐。
...
```text
versions/v1.52/firmware/stm_unload_zone_recognition_v152.elf
```
