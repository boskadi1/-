...
static void UpdateVisionTask(void)
{
  switch (gCar.sys.stage)
  {
    ...
    case CAR_STAGE_LINE_NAVIGATE:
      ...
      {
        /* Do not brake at the unloading-area entrance. Keep the line camera
         * pose and creep forward until the circular drop point is ready. */
        SetStage(CAR_STAGE_UNLOAD_SEARCH);
        gCar.sys.action = CAR_ACTION_SLOW_FORWARD;
      }
      else
    ...
    case CAR_STAGE_UNLOAD_ALIGN:
      ...
      {
        /* Never unload from the old line-view position. Hold still until a
         * fresh circle packet is received in the pickup camera pose. */
        s_target_ready_active = 0U;
        gCar.sys.action = CAR_ACTION_STOP;
      }
      break;
