...
static void UpdateVisionTask(void)
{
  switch (gCar.sys.stage)
  {
    ...
    case CAR_STAGE_LOAD_CARGO:
      ...
      {
        /* Switch OpenMV immediately after pickup confirmation. Previously
         * MODE,LINE was delayed until the two-second exit drive finished,
         * leaving the IDE image in RGB and making the switch look broken. */
    ...
    case CAR_STAGE_LEAVE_LOAD_ZONE:
      /* The first complete transverse-line passage is the start marker.
       * Keep the 10/10 line view, cross it at base speed, and only then
       * enable normal line navigation. */
      Vision_SetProcessingMode(VISION_PROCESS_LINE);
      if (CameraServo_IsLineViewReady() == 0U)
      {
        gCar.sys.action = CAR_ACTION_STOP;
      }
      else if ((gCar.vision.transverse_passed_count >= 1U) &&
               (gCar.vision.transverse_active == 0U))
      {
        gCar.sys.action = CAR_ACTION_STOP;
        SetStage(CAR_STAGE_LINE_ENTRY);
      }
      else if (StageElapsed(CAR_START_LINE_CROSS_TIMEOUT_MS))
      {
        /* Do not accidentally start tracking before the start marker. */
        gCar.sys.action = CAR_ACTION_STOP;
      }
      else
      {
    ...
    case CAR_STAGE_LINE_NAVIGATE:
      ...
      else if (gCar.vision.transverse_count >= 2U)
      {
        /* The second transverse line is the unloading entrance. Keep the
         * camera at 10/10 and continue straight while looking ahead for the
         * black circle before the red cargo blocks it. */
        SetStage(CAR_STAGE_UNLOAD_SEARCH);
        gCar.sys.action = CAR_ACTION_SLOW_FORWARD;
      }
      else
    ...
    case CAR_STAGE_UNLOAD_ALIGN:
       ...
       * Restore normal RGB, tilt to the pickup pose, then ignore the
       * occluded circle and finish with calibrated forward time. */
      Vision_SetProcessingMode(VISION_PROCESS_RED);
      if (CameraServo_IsPickupViewReady() == 0U)
      {
        gCar.sys.action = CAR_ACTION_STOP;
