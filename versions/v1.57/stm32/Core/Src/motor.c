...
static uint32_t s_last_encoder_sample_tick;
static int8_t s_left_pid_direction;
static int8_t s_right_pid_direction;
static PidController_t s_left_speed_pid;
static PidController_t s_right_speed_pid;
...
static int16_t AbsSpeed(int16_t speed)
{
  return (speed < 0) ? (int16_t)(-speed) : speed;
}

static int8_t SpeedDirection(int16_t speed)
{
  if (speed > 0) return 1;
  if (speed < 0) return -1;
  return 0;
...
static int16_t CalculateClosedLoopOutput(int16_t target,
                                         int16_t feedback,
                                         uint8_t encoder_valid,
                                         PidController_t *pid,
                                         int8_t *last_direction)
{
  int8_t direction = SpeedDirection(target);
  int16_t target_magnitude;
  int16_t feedback_magnitude;
  ...
  if ((encoder_valid == 0U) || (*last_direction != direction))
  {
    Pid_Reset(pid);
  ...
  target_magnitude = AbsSpeed(target);
  feedback_magnitude = AbsSpeed(feedback);
  correction = Pid_Update(pid, (float)target_magnitude,
                          (float)feedback_magnitude,
                          (float)ENCODER_SAMPLE_MS / 1000.0f);
  output_magnitude = target_magnitude +
      (int32_t)((correction >= 0.0f) ?
                (correction + 0.5f) : (correction - 0.5f));
  if (output_magnitude > MOTOR_SPEED_MAX)
    output_magnitude = MOTOR_SPEED_MAX;
  if (output_magnitude < MOTOR_EFFECTIVE_MIN_SPEED)
    output_magnitude = MOTOR_EFFECTIVE_MIN_SPEED;

  return (direction > 0) ? (int16_t)output_magnitude :
                           (int16_t)(-output_magnitude);
}
...
{
  Motor_HardwareInit();
  Pid_Init(&s_left_speed_pid, MOTOR_SPEED_PID_KP, MOTOR_SPEED_PID_KI,
           MOTOR_SPEED_PID_KD, -MOTOR_SPEED_PID_CORRECTION_MAX,
           MOTOR_SPEED_PID_CORRECTION_MAX);
  Pid_Init(&s_right_speed_pid, MOTOR_SPEED_PID_KP, MOTOR_SPEED_PID_KI,
           MOTOR_SPEED_PID_KD, -MOTOR_SPEED_PID_CORRECTION_MAX,
           MOTOR_SPEED_PID_CORRECTION_MAX);
  s_left_applied = 0;
  s_right_applied = 0;
  s_last_ramp_tick = HAL_GetTick();
  s_last_encoder_sample_tick = gCar.motion.encoder_sample_tick;
  s_left_pid_direction = 0;
  s_right_pid_direction = 0;
...
void Motor_Update(void)
{
  ...
  if ((MOTOR_SPEED_CLOSED_LOOP_ENABLE != 0U) &&
      (gCar.motion.encoder_sample_tick != s_last_encoder_sample_tick))
  {
    s_last_encoder_sample_tick = gCar.motion.encoder_sample_tick;
    s_last_ramp_tick = HAL_GetTick();
    if (gCar.motion.left_encoder_valid != 0U)
    {
      s_left_applied = CalculateClosedLoopOutput(
          gCar.motion.left_set_speed, gCar.motion.left_feedback_speed,
          1U, &s_left_speed_pid, &s_left_pid_direction);
    }
    else
    {
      Pid_Reset(&s_left_speed_pid);
      s_left_pid_direction = 0;
      s_left_applied = ApproachSpeed(s_left_applied,
                                     gCar.motion.left_set_speed);
    }
    if (gCar.motion.right_encoder_valid != 0U)
    {
      s_right_applied = CalculateClosedLoopOutput(
          gCar.motion.right_set_speed, gCar.motion.right_feedback_speed,
          1U, &s_right_speed_pid, &s_right_pid_direction);
    }
    else
    {
      Pid_Reset(&s_right_speed_pid);
      s_right_pid_direction = 0;
      s_right_applied = ApproachSpeed(s_right_applied,
                                      gCar.motion.right_set_speed);
    }
    Motor_ApplyLeft(s_left_applied);
    Motor_ApplyRight(s_right_applied);
  }
  else if ((HAL_GetTick() - s_last_ramp_tick) >= MOTOR_RAMP_INTERVAL_MS)
  {
    s_last_ramp_tick = HAL_GetTick();
    /* Before encoder pulses are available, retain the proven open-loop ramp.
     * A disconnected encoder therefore cannot drive PWM to 100%. */
    if ((MOTOR_SPEED_CLOSED_LOOP_ENABLE == 0U) ||
