...
# first transverse black line is the start line and the second is the
# unloading entrance. The circle is captured early, before the red cargo
...
# v2.7: two transverse-line events and predictive-unload handoff.
...
VERSION = "SENSOR_CROSS_PREDICT2.7"
...
UNLOAD_DETECT_ARM_DELAY_MS = 300
    ...
    while True:
        ...
        if time.ticks_diff(now_ms, last_send_ms) >= SEND_INTERVAL_MS:
            ...
            uart.write("MV,UNLOAD_ENTRY,%d\r\n" %
                       (1 if transverse_count >= 2 else 0))
