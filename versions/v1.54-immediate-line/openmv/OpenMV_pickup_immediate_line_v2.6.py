...
# After the visual pickup transition is confirmed, this script immediately
# switches to grayscale/binary continuous fitted-line processing from v1.9.
# MODE,LINE from STM32 remains supported as a second switching path. At the
...
#                    MV,UNLOAD_ENTRY,...
# v2.6: immediate autonomous RGB-to-line switch after pickup.
...
VERSION = "SENSOR_PICKUP_IMMEDIATE_LINE2.6"
...
def main():
    ...
    pickup_ready_seen = False
    pickup_close_confirm_count = 0
    pickup_line_forced = False

    while True:
        clock.tick()
        requested_mode, command_buffer = poll_mode_command(
            uart, processing_mode, command_buffer)
        # Before the STM32 finishes its claw/lift state it may still repeat an
        # old MODE,RED packet. Once pickup has been visually confirmed, do not
        # allow that stale packet to cancel the immediate line-mode switch.
        if pickup_line_forced and requested_mode == MODE_RED:
            requested_mode = MODE_LINE
        ...
        if processing_mode == MODE_RED:
            ...
            # GRIP_READY is the position that starts the STM32 pickup action.
            # Once the closing claw carries the red block below that window,
            # GRIP_TOO_CLOSE is observed. Three consecutive frames confirm
            # that transition and switch the image immediately without relying
            # on the STM32-to-OpenMV UART direction.
            if valid and grip_state == GRIP_READY:
                pickup_ready_seen = True
                pickup_close_confirm_count = 0
            elif (valid and pickup_ready_seen and
                  grip_state == GRIP_TOO_CLOSE):
                pickup_close_confirm_count += 1
            else:
                pickup_close_confirm_count = 0

            if pickup_close_confirm_count >= PICKUP_CLOSE_CONFIRM_FRAMES:
                pickup_line_forced = True
                processing_mode = MODE_LINE
                configure_camera(processing_mode)
                track_locked = False
                filter_valid = False
                pending_count = 0
                ...
                unload_entry_latched = False
                line_mode_start_ms = time.ticks_ms()
                print("MODE=LINE AUTO_AFTER_PICKUP")
                continue
