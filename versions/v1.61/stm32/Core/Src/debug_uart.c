...
void DebugUart_PrintStatus(void)
{
  int len = snprintf(s_debug_buf, sizeof(s_debug_buf),
                     "STAT,V=%s,MODE=%s,STAGE=%s,ACT=%s,VM=%u/%u/%u,US=%u/%u/%u/%u,V=%u%u%u%u,LINE=%d/%d/%u/%u/%d,D=%d,LOAD=%d/%d/%lu/%u,TAG=%u/%d/%d/%lu/%u,VRX=%lu/%lu/%lu/%lu,CAM=%d/%d>%d/%d,YAW=%ld.%02ld,TYAW=%d,ERR=%d\r\n",
                     CAR_SOFTWARE_VERSION,
                     CarData_ModeName(gCar.sys.mode),
                     CarData_StageName(gCar.sys.stage),
                     CarData_ActionName(gCar.sys.action),
                     (unsigned int)Vision_GetProcessingMode(),
                     (unsigned int)gCar.vision.reported_mode,
                     (unsigned int)gCar.vision.mode_ack_valid,
