...
typedef enum
{
  ...
  GRAB_STATE_DROPOFF_RETURN_INIT
} GrabState_t;
...
void GrabServo_Update(void)
{
  ...
  switch (s_state)
  {
    ...
    case GRAB_STATE_DROPOFF_OPEN:
      ...
      {
        gCar.grab.has_object = 0U;
        s_state = GRAB_STATE_DROPOFF_RETURN_INIT;
        s_state_tick = now;
        /* The cargo has been released. Return both axes to the exact same
         * calibrated pose used after power-up before completing the task. */
        SetClawAngle(GRAB_CLAW_INIT_DEG);
        SetLiftAngle(GRAB_LIFT_INIT_DEG);
      }
      break;

    case GRAB_STATE_DROPOFF_RETURN_INIT:
      if (elapsed >= GRAB_ACTION_DELAY_MS)
      {
        gCar.grab.busy = 0U;
        s_state = GRAB_STATE_IDLE;
