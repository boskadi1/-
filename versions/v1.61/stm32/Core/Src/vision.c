...
static void SendProcessingMode(void)
{
  ...
  if (s_processing_mode == VISION_PROCESS_LINE)
  {
    command = "MODE,LINE\r\n";
  }
  else if (s_processing_mode == VISION_PROCESS_START)
  {
    command = "MODE,START\r\n";
  }
  else if (s_processing_mode == VISION_PROCESS_UNLOAD)
  {
    command = "MODE,UNLOAD\r\n";
  }
  else
...
static uint8_t Vision_ParseLine(const char *line)
{
  ...
  int transverse_active = 0;
  int transverse_passed_count = 0;
  int start_line_active = 0;
  int start_line_passed = 0;
  char mode_name[12];
  uint32_t now = HAL_GetTick();

  if (sscanf(line, "MV,MODE,%11s", mode_name) == 1)
  {
    VisionProcessingMode_t reported_mode;

    if (strcmp(mode_name, "LINE") == 0)
      reported_mode = VISION_PROCESS_LINE;
    else if (strcmp(mode_name, "START") == 0)
      reported_mode = VISION_PROCESS_START;
    else if (strcmp(mode_name, "UNLOAD") == 0)
      reported_mode = VISION_PROCESS_UNLOAD;
    else if (strcmp(mode_name, "RED") == 0)
      reported_mode = VISION_PROCESS_RED;
    else
      return 0U;

    gCar.vision.reported_mode = (uint8_t)reported_mode;
    gCar.vision.mode_ack_valid = 1U;
    gCar.vision.last_mode_ack_tick = now;
    return 1U;
  }
  ...
  if (sscanf(line, "MV,CROSS,%d,%d,%d", &transverse_count,
             &transverse_active, &transverse_passed_count) == 3)
  {
    if (transverse_count < 0) transverse_count = 0;
    if (transverse_count > 255) transverse_count = 255;
    if (transverse_passed_count < 0) transverse_passed_count = 0;
    if (transverse_passed_count > 255) transverse_passed_count = 255;
    gCar.vision.transverse_count = (uint8_t)transverse_count;
    gCar.vision.transverse_active =
        (transverse_active != 0) ? 1U : 0U;
    gCar.vision.transverse_passed_count =
        (uint8_t)transverse_passed_count;
    gCar.vision.last_transverse_tick = now;
    return 1U;
  }

  if (sscanf(line, "MV,START_LINE,%d,%d", &start_line_active,
             &start_line_passed) == 2)
  {
    gCar.vision.start_line_active =
        (start_line_active != 0) ? 1U : 0U;
    gCar.vision.start_line_passed =
        (start_line_passed != 0) ? 1U : 0U;
    gCar.vision.last_start_line_tick = now;
    return 1U;
  }
...
void Vision_Init(void)
{
  ...
  gCar.vision.reported_mode = (uint8_t)VISION_PROCESS_RED;
  gCar.vision.mode_ack_valid = 0U;
  gCar.vision.last_mode_ack_tick = 0U;
...
void Vision_SetProcessingMode(VisionProcessingMode_t mode)
{
  if (mode != s_processing_mode)
  {
    ...
    /* Every processing-mode change changes the image geometry or colour
     * space. Never let an unload result from the old view drive the car. */
    gCar.vision.unload_seen = 0U;
    gCar.vision.unload_position_state = CAR_UNLOAD_STATE_NOT_READY;
    gCar.vision.unload_position_valid = 0U;
    if (mode != VISION_PROCESS_LINE)
    {
...
{
  return s_processing_mode;
}

uint8_t Vision_IsModeConfirmed(VisionProcessingMode_t mode)
{
  if (gCar.vision.mode_ack_valid == 0U) return 0U;
  if ((HAL_GetTick() - gCar.vision.last_mode_ack_tick) >
      CAR_VISION_TIMEOUT_MS) return 0U;
  return (gCar.vision.reported_mode == (uint8_t)mode) ? 1U : 0U;
}
...
void Vision_Update(void)
{
  ...
  if ((HAL_GetTick() - gCar.vision.last_transverse_tick) >
      CAR_VISION_TIMEOUT_MS)
  {
    /* Counts are mission history; only the instantaneous presence expires. */
    gCar.vision.transverse_active = 0U;
  }

  if ((HAL_GetTick() - gCar.vision.last_start_line_tick) >
      CAR_VISION_TIMEOUT_MS)
  {
    gCar.vision.start_line_active = 0U;
  }
