...
static int32_t s_line_integral_accumulator;
...
static uint8_t s_unload_predict_started;
static uint32_t s_unload_predict_tick;
static uint32_t s_load_exit_tick;
...
static void SetStage(CarStage_t stage)
{
  if (gCar.sys.stage != stage)
  {
    ...
    {
      s_unload_predict_started = 0U;
      s_unload_predict_tick = HAL_GetTick();
    }
    if (stage == CAR_STAGE_LEAVE_LOAD_ZONE)
    {
      s_load_exit_tick = HAL_GetTick();
    }
  }
}
...
static void ResetLineIntegral(void)
{
  s_line_integral_accumulator = 0;
  gCar.motion.line_i_correction = 0;
}

static int16_t CalculateLineIntegral(int16_t error)
{
  int32_t i_correction;

  s_line_integral_accumulator += error;
  if (s_line_integral_accumulator > CAR_LINE_I_ACCUMULATOR_MAX)
    s_line_integral_accumulator = CAR_LINE_I_ACCUMULATOR_MAX;
  if (s_line_integral_accumulator < -CAR_LINE_I_ACCUMULATOR_MAX)
    s_line_integral_accumulator = -CAR_LINE_I_ACCUMULATOR_MAX;

  i_correction = s_line_integral_accumulator * CAR_LINE_KI_NUM /
                 CAR_LINE_KI_DEN;
  if (i_correction > CAR_LINE_I_TERM_MAX)
    i_correction = CAR_LINE_I_TERM_MAX;
  if (i_correction < -CAR_LINE_I_TERM_MAX)
    i_correction = -CAR_LINE_I_TERM_MAX;
  gCar.motion.line_i_correction = (int16_t)i_correction;
  return (int16_t)i_correction;
}
...
static CarAction_t MakeLineAction(void)
{
  ...
  else
  {
    ...
    correction += CalculateLineIntegral(gCar.vision.line_offset);
...
static void UpdateVisionTask(void)
{
  switch (gCar.sys.stage)
  {
    ...
    case CAR_STAGE_LOAD_CARGO:
      ...
      {
        /* Initial transverse-line recognition is disabled. Keep normal RGB
         * and leave the loading area by a fixed four-second straight run. */
        Vision_SetProcessingMode(VISION_PROCESS_RED);
    ...
    case CAR_STAGE_LEAVE_LOAD_ZONE:
      /* Drive straight at the calibrated 83/70 base speeds for four seconds.
       * The camera moves independently. Do not wait for an exact servo-angle
       * comparison here, because that could hold this state forever. */
      Vision_SetProcessingMode(VISION_PROCESS_RED);
      if ((HAL_GetTick() - s_load_exit_tick) >= CAR_LOAD_ZONE_EXIT_MS)
      {
        gCar.sys.action = CAR_ACTION_STOP;
    ...
    case CAR_STAGE_LINE_ENTRY:
      /* Reassert LINE continuously; Vision_Update also retransmits it every
       * 500 ms. Wait for OpenMV's explicit acknowledgement before driving
       * from line data. */
      Vision_SetProcessingMode(VISION_PROCESS_LINE);
      if (Vision_IsModeConfirmed(VISION_PROCESS_LINE) == 0U)
      {
        gCar.sys.action = CAR_ACTION_STOP;
      }
      else if (CameraServo_IsLineViewReady() == 0U)
      {
        gCar.sys.action = CAR_ACTION_STOP;
      }
    ...
    case CAR_STAGE_LINE_NAVIGATE:
      ...
      else if (StageElapsed(CAR_UNLOAD_DETECT_GUARD_MS) &&
               (gCar.vision.transverse_count >= 1U))
      {
        /* At the unloading entrance immediately leave the inverted line
         * image. Keep the 10/10 camera pose and find the black marker in
         * normal RGB so it remains distinct from the red cargo. */
    ...
    case CAR_STAGE_UNLOAD_ALIGN:
      /* The early circle handoff has already fixed the release trajectory.
       * Keep the RGB unloading mode, tilt to the pickup pose, then ignore
       * the occluded circle and finish with calibrated forward time. */
      Vision_SetProcessingMode(VISION_PROCESS_UNLOAD);
      if (CameraServo_IsPickupViewReady() == 0U)
      {
        gCar.sys.action = CAR_ACTION_STOP;
      ...
      {
        uint32_t predict_elapsed;
        if (s_unload_predict_started == 0U)
        {
          s_unload_predict_started = 1U;
          s_unload_predict_tick = HAL_GetTick();
        }
        predict_elapsed = HAL_GetTick() - s_unload_predict_tick;
        if (predict_elapsed < CAR_UNLOAD_PREDICT_FORWARD_MS)
        {
          gCar.sys.action = CAR_ACTION_SLOW_FORWARD;
        }
        else if (predict_elapsed <
                 (CAR_UNLOAD_PREDICT_FORWARD_MS +
                  CAR_UNLOAD_PREDICT_SETTLE_MS))
        {
          gCar.sys.action = CAR_ACTION_STOP;
        }
        else
    ...
    case CAR_STAGE_UNLOAD_SEARCH:
      ...
      else if (StageElapsed(CAR_UNLOAD_RGB_SETTLE_MS) == 0U)
      {
        /* OpenMV resets the sensor and lets RGB exposure/white balance
         * settle. Hold the car still until fresh RGB frames are available. */
        gCar.sys.action = CAR_ACTION_STOP;
      }
      else if (FrontUnloadEmergency())
      {
      ...
      {
        /* Flip down immediately at the early calibrated window. Waiting for
         * another post-flip frame is unreliable because cargo occludes it. */
        gCar.sys.action = CAR_ACTION_STOP;
        SetStage(CAR_STAGE_UNLOAD_ALIGN);
      }
      else if ((gCar.vision.unload_seen != 0U) &&
      ...
      else if (StageElapsed(CAR_UNLOAD_VIEW_SEARCH_TIMEOUT_MS))
      {
        /* Fail safe: do not drive indefinitely if the early circle event
         * was missed or its threshold needs recalibration. */
        gCar.sys.action = CAR_ACTION_STOP;
      }
      else
      {
        /* Slow straight approach (70/60); no pan scanning and no camera
         * tilt until the early circle window is reached. */
        gCar.sys.action = CAR_ACTION_SLOW_FORWARD;
      }
      break;
