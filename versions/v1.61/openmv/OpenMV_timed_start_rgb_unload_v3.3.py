...
# After pickup STM32 sends MODE,START. The normal RGB image is retained while
# the car crosses the first transverse black line. Only after the complete
# passage does STM32 send MODE,LINE and enable inverted fitted-line tracking.
# The next transverse line is the unloading entrance. The circle is captured
# early, before the red cargo
# occludes it; STM32 then finishes placement by calibrated time.
# STM32 -> OpenMV: MODE,RED, MODE,START, MODE,LINE or MODE,UNLOAD
# OpenMV -> STM32: MV,LOAD,... / MV,LINE,... / MV,UNLOAD,... /
#                    MV,CROSS,... / MV,UNLOAD_ENTRY,...
# v3.3: acknowledge every active processing mode to STM32.
...
VERSION = "SENSOR_TIMED_START_UNLOAD3.3"
...
UNLOAD_DETECT_ARM_DELAY_MS = 10000
...
UNLOAD_ENTRY_CLEAR_FRAMES = 3

# RGB start-line detector. It samples the live RGB image directly, so the IDE
# display stays in normal colour and no second QVGA framebuffer is allocated.
START_LINE_Y_MIN = 45
START_LINE_Y_MAX = 205
START_LINE_Y_STEP = 5
START_LINE_BASE_LUMA = 125
START_LINE_MAX_LUMA = 175
START_LINE_BACKGROUND_STEP = 16
START_LINE_CONTRAST = 45
START_LINE_MIN_DARK = 170
START_LINE_MIN_SIDE_DARK = 45

UNLOAD_ZONE_X_MIN = 110
UNLOAD_ZONE_X_MAX = 210
# Early handoff window at the 10/10 line-camera pose. Tune these Y values so
# READY is sent before the carried red block covers the black circle.
UNLOAD_ZONE_Y_MIN = 115
UNLOAD_ZONE_Y_MAX = 165
...
def capture_frame():
    try:
        ...
        # A mode change can occasionally collide with the first capture on
        # OV5640/OpenMV 5.0.0. Reclaim the heap and retry once, while printing
        # the original exception so a persistent hardware fault stays visible.
        print("SNAP_RETRY:", error)
        gc.collect()
        time.sleep_ms(50)
        return sensor.snapshot()
...
def configure_camera(mode):
    gc.collect()
    sensor.reset()
    if mode == MODE_RED or mode == MODE_UNLOAD or mode == MODE_START:
...
def poll_mode_command(uart, current_mode, command_buffer):
    ...
    while uart.any():
        ...
        if value == 10 or value == 13:
            ...
            elif command_buffer == "MODE,START":
                requested_mode = MODE_START
...
def find_unload_circle_rgb(img):
    ...
    for blob in blobs:
        ...
        # In RGB mode the LAB black threshold separates the ground marker
        # from the carried red cargo. Prefer the largest compact component.
    ...
    for y in range(UNLOAD_ENTRY_Y_MIN, UNLOAD_ENTRY_Y_MAX + 1,
        ...
        for x in range(IMG_WIDTH):
            ...
            if isinstance(pixel, (tuple, list)):
                is_white = (pixel[0] >= BINARY_LINE_MIN and
                            pixel[1] >= BINARY_LINE_MIN and
                            pixel[2] >= BINARY_LINE_MIN)
            else:
                is_white = pixel >= BINARY_LINE_MIN
            if is_white:
                white_count += 1
...
def pixel_luma(pixel):
    if isinstance(pixel, (tuple, list)):
        return (pixel[0] * 30 + pixel[1] * 59 +
                pixel[2] * 11) // 100
    return pixel


def detect_rgb_start_line(img):
    best_y = -1
    best_dark = 0
    best_left = 0
    best_right = 0
    best_threshold = START_LINE_BASE_LUMA

    for y in range(START_LINE_Y_MIN, START_LINE_Y_MAX + 1,
                   START_LINE_Y_STEP):
        brightest = 0
        for x in range(0, IMG_WIDTH, START_LINE_BACKGROUND_STEP):
            luma = pixel_luma(img.get_pixel((x, y)))
            if luma > brightest:
                brightest = luma

        threshold = clamp(brightest - START_LINE_CONTRAST,
                          START_LINE_BASE_LUMA,
                          START_LINE_MAX_LUMA)
        dark_count = 0
        left_count = 0
        right_count = 0
        ...
        if dark_count > best_dark:
            ...
            best_right = right_count
            best_threshold = threshold

    detected = (best_dark >= START_LINE_MIN_DARK and
                best_left >= START_LINE_MIN_SIDE_DARK and
                best_right >= START_LINE_MIN_SIDE_DARK)
    return (detected, best_y, best_dark, best_left,
            best_right, best_threshold)
...
def main():
    ...
    unload_entry_clear_count = 0
    unload_entry_active = False
    transverse_count = 0
    transverse_passed_count = 0
    start_line_confirm_count = 0
    start_line_clear_count = 0
    start_line_active = False
    ...
    while True:
        ...
        if requested_mode != processing_mode:
            ...
            if processing_mode == MODE_LINE:
                ...
                mode_name = "UNLOAD"
            elif processing_mode == MODE_START:
                mode_name = "START_RGB"
            else:
                mode_name = "RED"
            print("MODE=%s" % mode_name)

        now_mode_ms = time.ticks_ms()
        if time.ticks_diff(now_mode_ms, last_mode_ack_ms) >= MODE_ACK_INTERVAL_MS:
            last_mode_ack_ms = now_mode_ms
            if processing_mode == MODE_LINE:
                ack_name = "LINE"
            elif processing_mode == MODE_UNLOAD:
                ack_name = "UNLOAD"
        ...
        if processing_mode == MODE_START:
            img = capture_frame()
            # Sample the live RGB pixels directly. No image copy or extra
            # framebuffer is needed, and the IDE display remains normal RGB.
            (start_candidate, start_best_y, start_best_dark,
             start_best_left, start_best_right,
             start_best_threshold) = detect_rgb_start_line(img)
            if start_best_y >= 0:
                start_color = ((0, 255, 0) if start_candidate else
                               (255, 255, 0))
                img.draw_line((0, start_best_y, IMG_WIDTH - 1,
                               start_best_y), color=start_color, thickness=2)

            if start_line_active:
                if start_candidate:
                    start_line_clear_count = 0
                else:
                    start_line_clear_count += 1
                    if start_line_clear_count >= UNLOAD_ENTRY_CLEAR_FRAMES:
                        start_line_active = False
                        start_line_passed = True
                        start_line_clear_count = 0
            elif not start_line_passed:
                if start_candidate:
                    start_line_confirm_count += 1
                    if (start_line_confirm_count >=
                            UNLOAD_ENTRY_CONFIRM_FRAMES):
                        start_line_active = True
                        start_line_confirm_count = 0
                        start_line_clear_count = 0
                else:
                    start_line_confirm_count = 0
            ...
            if time.ticks_diff(now_ms, last_send_ms) >= SEND_INTERVAL_MS:
                last_send_ms = now_ms
                uart.write("MV,START_LINE,%d,%d\r\n" %
                           (1 if start_line_active else 0,
                            1 if start_line_passed else 0))

            if time.ticks_diff(now_ms, last_debug_ms) >= DEBUG_PRINT_INTERVAL_MS:
                last_debug_ms = now_ms
                print("START_%s FPS=%.1f CAND=%d ACTIVE=%d PASSED=%d "
                      "CONF=%d CLEAR=%d Y=%d DARK=%d L=%d R=%d TH=%d" %
                      (VERSION, clock.fps(), 1 if start_candidate else 0,
                       1 if start_line_active else 0,
                       1 if start_line_passed else 0,
                       start_line_confirm_count, start_line_clear_count,
                       start_best_y, start_best_dark, start_best_left,
                       start_best_right, start_best_threshold))
            continue

        if processing_mode == MODE_UNLOAD:
            img = capture_frame()
            unload_candidate = find_unload_circle_rgb(img)
            unload_blob = None
            unload_valid = 0
            unload_fill = 0
        ...
        if (line_mode_start_ms != 0 and
            ...
            if transverse_count >= 1:
                unload_candidate = find_unload_circle(img)

        if unload_entry_active:
            if unload_entry_candidate:
                unload_entry_clear_count = 0
            else:
                unload_entry_clear_count += 1
                if unload_entry_clear_count >= UNLOAD_ENTRY_CLEAR_FRAMES:
                    unload_entry_active = False
                    unload_entry_clear_count = 0
                    transverse_passed_count = transverse_count
        else:
            if unload_entry_candidate:
                unload_entry_confirm_count += 1
                if unload_entry_confirm_count >= UNLOAD_ENTRY_CONFIRM_FRAMES:
                    unload_entry_active = True
                    unload_entry_confirm_count = 0
                    unload_entry_clear_count = 0
                    transverse_count += 1
            else:
        ...
        if time.ticks_diff(now_ms, last_send_ms) >= SEND_INTERVAL_MS:
            ...
            uart.write("MV,UNLOAD,%d,%d,%d,%d,%d\r\n" %
                       (unload_horizontal, unload_vertical, unload_area,
                        unload_state, unload_valid))
            uart.write("MV,CROSS,%d,%d,%d\r\n" %
                       (transverse_count,
                        1 if unload_entry_active else 0,
                        transverse_passed_count))
            uart.write("MV,UNLOAD_ENTRY,%d\r\n" %
                       (1 if transverse_count >= 1 else 0))
        ...
        if time.ticks_diff(now_ms, last_debug_ms) >= DEBUG_PRINT_INTERVAL_MS:
            ...
            print("LINE_%s FPS=%.1f NEAR=%d FAR=%d OFF=%d HEAD=%d "
                  ...
                  "CIRCLE=%d STATE=%d AREA=%d FILL=%d CONF=%d "
                  "CROSS=%d ACTIVE=%d PASSED=%d ENTRY_CONF=%d" %
                  (VERSION, clock.fps(), near_x, far_x, filtered_offset,
                   ...
                   unload_confirm_count, transverse_count,
                   1 if unload_entry_active else 0,
                   transverse_passed_count, unload_entry_confirm_count))
