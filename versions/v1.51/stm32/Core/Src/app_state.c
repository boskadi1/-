...
static CarAction_t MakeTargetApproachAction(uint8_t load_target, uint8_t *ready)
{
  ...
  *ready = 0U;
  ...
  if (bearing_deg > CAR_TARGET_ALIGN_DEADZONE_DEG)
  {
    s_target_ready_active = 0U;
    return CAR_ACTION_TURN_RIGHT;
  }
...
static void UpdateVisionTask(void)
{
  switch (gCar.sys.stage)
  {
    ...
    case CAR_STAGE_LINE_NAVIGATE:
      ...
      {
        /* The circular drop point may not be visible from the line pose.
         * Switch to the calibrated pickup/unload view as soon as the wide
         * unloading-area entrance is confirmed, then search from that view. */
        SetStage(CAR_STAGE_UNLOAD_ALIGN);
        gCar.sys.action = CAR_ACTION_STOP;
      }
      else
    ...
    case CAR_STAGE_UNLOAD_ALIGN:
      /* Changing from the line view to the pickup view changes the apparent
       * circle position. Re-acquire and align the car from the fixed 10/30
       * camera pose; only unload after the new view remains ready. */
      if (CameraServo_IsPickupViewReady() == 0U)
      {
        s_target_ready_active = 0U;
        gCar.sys.action = CAR_ACTION_STOP;
      ...
      {
        /* Search for the circle only after the camera reaches 10/30. This
         * removes the old deadlock which required the circle to be READY in
         * the 10/10 line view before the camera was allowed to move. */
        s_target_ready_active = 0U;
