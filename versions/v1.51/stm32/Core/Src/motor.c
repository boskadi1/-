...
void Motor_Update(void)
{
  ...
  switch (gCar.sys.action)
  {
    ...
    case CAR_ACTION_RESTORE_HEADING:
    {
      ...
      if (gCar.sys.stage == CAR_STAGE_UNLOAD_ALIGN)
      {
        left_speed = (int16_t)(-MOTOR_LOAD_APPROACH_LEFT_SPEED);
        right_speed = MOTOR_LOAD_APPROACH_RIGHT_SPEED;
      }
      else
      {
    ...
    case CAR_ACTION_TURN_RIGHT:
    {
      ...
      if (gCar.sys.stage == CAR_STAGE_UNLOAD_ALIGN)
      {
        left_speed = MOTOR_LOAD_APPROACH_LEFT_SPEED;
        right_speed = (int16_t)(-MOTOR_LOAD_APPROACH_RIGHT_SPEED);
      }
      else
      {
        Motor_GetInPlaceTurnTargets(1, 0, &left_speed, &right_speed);
